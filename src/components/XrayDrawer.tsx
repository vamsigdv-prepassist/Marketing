...
import { X, Network, Globe, Target, Loader2, BookOpen, BookText, TrendingUp, Leaf, Cpu } from "lucide-react";
import InteractiveMCQList from "./InteractiveMCQList";
...
export default function XrayDrawer({ 
  ...
  const getHistoryTabConfig = () => {
    const s = (subject || "history").toLowerCase();
    if (s === "polity") {
      return { icon: <BookOpen className="w-4 h-4" />, label: "Polity Static" };
    }
    if (s === "economy") {
      return { icon: <TrendingUp className="w-4 h-4" />, label: "Economy Static" };
    }
    if (s === "geography") {
      return { icon: <Globe className="w-4 h-4" />, label: "Geography Static" };
    }
    if (s === "environment") {
      return { icon: <Leaf className="w-4 h-4" />, label: "Environment Static" };
    }
    if (s === "science") {
      return { icon: <Cpu className="w-4 h-4" />, label: "Science Static" };
    }
    return { icon: <BookOpen className="w-4 h-4" />, label: "History" };
  };

  const getHistoryHeaderLabel = () => {
    const s = (subject || "history").toLowerCase();
    if (s === "polity") return "Constitutional Evolution";
    if (s === "economy") return "Economic Background";
    if (s === "geography") return "Geographical Context";
    if (s === "environment") return "Environmental Context";
    if (s === "science") return "Scientific Background";
        ...
        <>
          ...
          >
             <div className="p-6 border-b border-slate-100 flex items-start justify-between bg-slate-50 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl mix-blend-multiply pointer-events-none"></div>
                <div>
             ...
             <div className="flex px-6 pt-4 gap-4 border-b border-slate-100 overflow-x-auto whitespace-nowrap scrollbar-hide">
                ...
                >
                  <div className="flex items-center gap-2">{getHistoryTabConfig().icon} {getHistoryTabConfig().label}</div>
                    ...
                    {activeTab === "HISTORY" && (
                      <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                         ...
                         <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                           {getHistoryTabConfig().icon}
                           {getHistoryHeaderLabel()}
                         </h3>
                    ...
                    {activeTab === "PRELIMS" && (
                      <div className="space-y-4">
                         <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2 mb-2">
                           ...
                           Expected Prelims MCQs (5 Questions)
                         </h3>
                         <InteractiveMCQList rawInput={xrayData.prelims_practice} />
                      </div>
                    )}
