...
export function subscribeMainsQuestions(language: "English" | "Hindi", callback: (questions: MainsQuestion[]) => void) {
  const q = query(
    collection(db, "mains_questions"),
    where("language", "==", language)
  );
  ...
  return onSnapshot(q, (snap) => {
    ...
    callback(questions.sort((a: any, b: any) => {
    ...
    console.error("Crash executing Mains Question subscription:", error);
    callback([]);
  });
}
