...
import { addQuestion, bulkAddQuestions, fetchQuestions, subscribeQuestions, deleteQuestion, Question } from "@/lib/questions";
import { formatUPSC } from "@/lib/formatUPSC";

// Universal CSV Parser: Automatically detects commas, semicolons, or tabs
function parseCSV(text: string): string[][] {
  const lines = text.split(/\r?\n/).filter(l => l.trim().length > 0);
  if (lines.length === 0) return [];

  // Heuristic: Check first line for most common delimiter
  const delimiters = [",", ";", "\t"];
  let bestDelimiter = ",";
  let maxCols = 0;
...
export default function QuestionBankAdmin() {
  ...
  useEffect(() => {
    setIsLoading(true);
    const unsubscribe = subscribeQuestions(10, adminLanguage, (data) => {
