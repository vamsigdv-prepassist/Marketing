...
export default function AdminLogin() {
  ...
  const handleAdminAuth = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Authenticate via Firebase first to prove identity
      const userCredential = await signInWithEmailAndPassword(auth, email, password);
      const user = userCredential.user;

      // 2. Cross-reference Firestore users table for assigned Role clearance
      ...
      if (!isAssignedAdmin && !isStaticAdmin) {
         await signOut(auth); // Revoke tokens immediately
         setLoading(false);
      ...
      setTimeout(() => {
         window.location.href = "/"; // Root is the dashboard in standalone Admin app
      }, 800);
      ...
      >
         ...
         <div className="flex flex-col items-center mb-10 text-center">
            <div className="bg-white p-3 rounded-2xl mb-8 shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:scale-105 transition-transform duration-500">
               <Image src="/1024x1024.png" unoptimized quality={100} alt="PrepAssist Platform Logo" width={160} height={50} className="object-contain" priority />
            </div>
